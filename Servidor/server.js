...
const booksRouter = require('./booksRouter');  // Asegúrate de que el path sea correcto
app.use('/api', booksRouter);  // Monta el router para que las peticiones lleguen a '/api/books'

app.put('/api/multas/:id', (req, res) => {
  const idMulta = req.params.id;
  const estatus = req.body.Estatus;

  // Actualizar el estatus de la multa en la base de datos
  const query = 'UPDATE Multas SET Estatus = ? WHERE IdMulta = ?';
  pool.query(query, [estatus, idMulta], (error, results) => {
    if (error) {
      return res.status(500).send('Error al actualizar la multa');
    }
    res.send('Multa actualizada exitosamente');
  });
});

app.patch('/multaPago/:id', (req, res) => {
  const idMulta = req.params.id;
  const { Estatus } = req.body;

  // Actualizar el estado de la multa en la base de datos
  const query = 'UPDATE Multas SET Estatus = ? WHERE IdMulta = ?';
  pool.query(query, [Estatus, idMulta], (error, results) => {
    if (error) {
      return res.status(500).send('Error al actualizar el estado de la multa');
    }
    res.send('Multa actualizada exitosamente');
  });
});
...
const eventosService = require('./eventosService'); // Cambia a eventosService si el archivo se llama así
app.use(bodyParser.json());
app.use(cors());
app.use('/eventos',eventosService);
